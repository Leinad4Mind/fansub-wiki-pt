...
> Ladies and Mentlegen, I introduce you to YIYAGTFA: Isn't Yet Another Gradient Tool For Aegisub!

No, wait, I decided to call it ASSgrad.


## ASSgrad! ##
And that's because it makes gradients. They are a lot of ASS.
Its exciting feature is that it can do diagonal gradients and it supports alpha as well.
This is cool, but can mean even more ASS. The amount of ASS generated by this script can make renderers
perform like ASS, especially if you're trying to make awesome ASS or avoid banding that totally looks like ASS.
...
Once all the features are usable, however, here is what the interface shall be:
At the beginning of the Effect field of each desired line, enter this:
<Ag>(1c:2c:3c:4c;1a:2a:3a:4a;BandSize.BandOverlap.Left.Top.Right.Bottom)

where 1c is a dot delimited list of colors in the format of &HBBGGRR& or #RRGGBB,
where e.g. BB is a hexadecimal color value in the range 00–FF for blue.
1a is a dot delimited list of alpha values, and must be in decimal format from 0–255.
ASSgrad will iterate backwards across all of the lines in the script,
collecting the ones that match the proper pattern in the Effect field,
namely ^<Ag>\(.+?\). It will go through each collected line, creating the gradient for each one,
and deleting the relevant contents of the Effect field.

I currently have no plans for a tool to undo gradients,
...
BandOverlap is the amount of overlap between adjacent bands in pixels.
By default it will be set to the same value as BandSize (that is, its default default is 4 pixels,
but if you set BandSize and not BandOverlap, then BandOverlap will default to whatever you set as BandSize).

Left, Top, Right, and Bottom are manual expansions for the different edges of the line bounding box.
You should use them if the line does not fit all the way into the generated clips.
Positive values will always cause an expansion, and negative a contraction.
...
<Ag>(&HFF0000&.#00FF00;;10..5) - Creates the same gradient as above, but it uses a band size of 10 pixels and has a left side clipbox expansion of 5 pixels.
								 Also note that # is not required for rgb color formatting (that is, #0000FF and 0000FF are equivalent (and also very blue)).

<Ag>(&HFF0000&.&H00FF00&::&H00FF00&.&HFF0000&;:::0.180.255) - Should be valid, and will create a gradient on color 1 (main color), from blue to green,
															  a gradient on color 3 (outline), from green to blue, and an alpha gradient on color 4 (shadow)
															  that goes from opaque, to translucent, to fully transparent.
...
script_name = "Gradiente de ASS"
script_description = "O DERRADEIRO MÉTODO PARA REBENTARES COM O TEU SCRIPT FINALMENTE CHEGOU"
...
function GetSizeOfVectorObject(vect) -- doesn't actually work with an5 stuff silly me
  ...
  local function comparex(x)
    if x > xmax then
      xmax = x; aegisub.log(4,'Novo: xmax: %g\n',xmax)
    end
    if x < xmin then
      xmin = x; aegisub.log(4,'Novo: xmin: %g',xmin)
    end
  end
  local function comparey(y)
    if y > ymax then
      ymax = y; aegisub.log(4,'Novo: ymax: %g\n',ymax)
    end
    if y < ymin then
      ymin = y; aegisub.log(4,'Novo: ymin: %g',ymin)
    end
  end
  ...
  local function cubic(str)
    ...
    aegisub.log(4,'Cúbico: %s %s %s %s %s %s\n',x[2],y[2],x[3],y[3],x[4],y[4])
    for i = 2,4 do
...
function GetInfo(line,options) -- because CamelCase
  for k, v in pairs(header) do
    ...
    aegisub.log(5,"Linha %d: %s colocada a %s (do cabeçalho)\n", line.num, v, tostring(line[k]))
  end
  ...
  if a then
    for k, v in pairs(patterns) do
      ...
      if _ then
        line[k] = tonumber(_)
        aegisub.log(5,"Linha %d: %s colocada a %g\n",line.num,k,_)
      end
    end
...
aegisub.register_macro(script_name,"PINTA COM GRADIENTE ESSE TEU ASS COMO SE NÃO HOUVESSE AMANHÃ", GatherLines)