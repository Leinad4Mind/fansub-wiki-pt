--[[
 Copyright (c) 2012-2013, Youka, tophf e Leinad4Mind
 All rights reserved®.
...
script_description = 'Adiciona expressões a todas as linhas, ou às linhas seleccionadas'
script_author = "Youka, tophf e Leinad4Mind"
script_version = "4.1"
script_modified = "20 Outubro 2013"

require('utils')

function add_tags(subs, sel, act)
  ...
  onetimeinit = function()
    ...
    scope = {
      all = 'Todas as Linhas',
      sel = 'Linhas Seleccionadas',
      cur = 'Linha Actual'
    }
    location = expandlist({
      {
        line = 'Começo da linha'
      },
      {
        first_in = '{...'
      },
    ...
    loadprompt = 'Itens anteriores...'
    btns = expandlist({
      {
        ok = '&Adicionar / reusar item escolhido'
      },
      {
        load = 'Carregar &de volta'
      },
      {
        cancel = '&Cancelar'
      }
    ...
    local dlg = {
      {
        ...
        name = 'text',
        text = config.text,
        hint = 'Introduza expressões a adicionar'
      },
      {
        'dropdown',
        0,
      ...
      },
      {
        ...
        items = _used,
        value = loadprompt,
        hint = 'Seleccione valor a usar ao invés do texto inserido'
      }
    }
    for _index_0 = 1, #dlg do
  ...
  cfgwrite = function()
    do
      ...
      if f then
        ...
        return aegisub.log('Erro ao escrever ' .. userconfigpath)
      end
    end
  end
  onetimeinit()
...
return aegisub.register_macro(script_name, script_description, add_tags)
